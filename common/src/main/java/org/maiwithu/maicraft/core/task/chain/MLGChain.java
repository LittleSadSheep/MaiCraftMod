...
public final class MLGChain implements Task, org.maiwithu.maicraft.task.reflex.Reflex {
    ...
    private String attentionMeans = "unknown";
    ...
    private TaskState clutch(LocalPlayer companion) {
        ...
        if (ground == null) {
            InputDriver.look(companion, companion.getYRot(), 90.0f);
            return TaskState.RUNNING;
        }
        ...
        if (bucket >= 0) {
            var selected = selection.select(companion, bucket);
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.RUNNING) return TaskState.RUNNING;
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.FAILED) return TaskState.FAILED;
            beginAttention(companion, "water_bucket",
                    "water may be placed temporarily and bucket recovery can fail");
            placed = waterLandsAt(companion, aim);
            reclaimTicks = RECLAIM_TICKS;
            action = Interaction.useInAir(companion, InteractionHand.MAIN_HAND, Interaction.Timing.once());
            pendingSave = "a water bucket";
            return TaskState.RUNNING;
        }
        int block = softBlockSlot(companion);
        if (block >= 0) {
            var selected = selection.select(companion, block);
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.RUNNING) return TaskState.RUNNING;
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.FAILED) return TaskState.FAILED;
            beginAttention(companion, "soft_block",
                    "one fall-dampening block may be consumed and remain in the world");
            action = Interaction.useBlock(companion, aim, InteractionHand.MAIN_HAND);
            pendingSave = "a soft block";
        }
        return TaskState.RUNNING;
    }

    /** 落进自己那摊水:等沉稳了,换空桶把水收回来。 */
    private TaskState reclaim(LocalPlayer companion) {
        if (action != null) return tickAction(companion);
        ...
        if (empty >= 0) {
            var selected = selection.select(companion, empty);
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.RUNNING) return TaskState.RUNNING;
            if (selected == org.maiwithu.maicraft.core.task.FirstPersonActionGate.Status.FAILED) return TaskState.FAILED;
            action = Interaction.useInAir(companion, InteractionHand.MAIN_HAND, Interaction.Timing.once());
        }
    ...
    private TaskState tickAction(LocalPlayer companion) {
        return switch (action.tick()) {
            case RUNNING -> TaskState.RUNNING;
            case FAILED -> {
                action = null;
                selection.reset();
                finishAttention(companion, "action_failed");
            ...
            case DONE -> {
                ...
                selection.reset();
                attentionActions++;
                if (pendingSave != null) { noteSave(companion, pendingSave); pendingSave = null; }
    ...
    private void noteSave(LocalPlayer companion, String means) {
        if (notedThisFall) return;
        notedThisFall = true;
        org.maiwithu.maicraft.core.Constants.LOG.info("[maicraft-reflex] broke a fall with {}", means);
    }

    private void beginAttention(LocalPlayer companion, String means, String consumptionRisk) {
        if (attentionActive) return;
        attentionActive = true;
        attentionMeans = means;
        attentionStartHealth = companion.getHealth();
        attentionActions = 0;
        GameplayAttentionMonitor.reflexStarted(
                id(), "rapid fall with imminent impact", "emergency item use",
                consumptionRisk, "fall damage or death if the clutch fails");
    }
    ...
    private void finishAttention(LocalPlayer companion, String outcome) {
        ...
        String resource = "water_bucket".equals(attentionMeans)
                ? (waterBucketSlot(companion) >= 0
                        ? "water bucket present after reflex"
                        : "water bucket recovery is not proven")
                : "soft block may remain placed";
        GameplayAttentionMonitor.reflexFinished(
                id(), outcome, attentionActions, resource,
                healthLost > 0.0F ? "health lost during reflex: " + healthLost : "no health loss observed");
        attentionActive = false;
        attentionMeans = "unknown";
        attentionActions = 0;
    }
     ...
     */
    private static BlockHitResult bucketRay(LocalPlayer companion, ClipContext.Fluid fluids) {
    ...
    private static BlockPos waterLandsAt(LocalPlayer companion, BlockHitResult hit) {
     ...
     */
    private static BlockPos groundBelow(LocalPlayer companion) {
