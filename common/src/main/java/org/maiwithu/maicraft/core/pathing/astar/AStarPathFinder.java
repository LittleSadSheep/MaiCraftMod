...
import org.maiwithu.maicraft.core.pathing.moves.Moves;
...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    private final CalculationContext.BorderSnapshot border;
    ...
    public AStarPathFinder(BlockPos realStart, int startX, int startY, int startZ,
        ...
        this.border = context.worldBorder;
    }

    /** (x,z) 格是否整格落在世界边界内。 */
    private boolean borderEntirelyContains(int x, int z) {
        return border == null || border.entirelyContains(x, z);
    }
    ...
    protected Optional<NavPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        CalculationContext.SearchConfig search = calcContext.searchConfig;
        int pathingMaxChunkBorderFetch = search.maxChunkBorderFetch();
        double minimumImprovement = search.minimumImprovementRepropagation() ? MIN_IMPROVEMENT : 0;
        // 节点硬上限:与时间预算同为循环出口,谁先到谁停。命中后落到下方 bestSoFar(...) 返回,
        // 与超时收尾完全一致(有可用半程即 SEGMENT,否则 FAILURE)。见 NavSettings#maxNodesPerSearch。
        int maxNodes = search.maxNodes();
        ...
        while (!openSet.isEmpty() && numNodes < maxNodes && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            for (Moves moves : allMoves) {
                ...
                if (moves.dynamicXZ && !calcContext.isLoaded(res.x, res.z)) {
                    numEmptyChunk++;
                    continue;
                }
