package org.maiwithu.maicraft.core.pathing.astar;
...
public abstract class AbstractNodeCostSearch {
    ...
    protected AbstractNodeCostSearch(BlockPos realStart, int startX, int startY, int startZ,
        ...
        CalculationContext.SearchConfig search = context.searchConfig;
        this.map = new Long2ObjectOpenHashMap<>(search.mapDefaultSize(), search.mapLoadFactor());
    }
    ...
    public synchronized PathCalcResult calculate(long primaryTimeout, long failureTimeout) {
        ...
        long tSearch = context.searchConfig.profile() ? System.nanoTime() : 0L;
        try {
            ...
            path = cutoffAtLoadedChunks(path);
            path = staticCutoff(path);
            ...
            isFinished = true;
            org.maiwithu.maicraft.core.pathing.util.NavProfiler.recordSearch(tSearch);
        }
    }

    protected abstract Optional<NavPath> calculate0(long primaryTimeout, long failureTimeout);

    private NavPath cutoffAtLoadedChunks(NavPath path) {
        if (!context.searchConfig.cutoffAtLoadBoundary()) {
            return path;
        }
        for (int i = 0; i < path.positions().size(); i++) {
            BlockPos pos = path.positions().get(i);
            if (!context.loadedTest.isLoaded(pos.getX(), pos.getZ())) {
                return new CutoffPath(path, i);
            }
        }
        return path;
    }

    private NavPath staticCutoff(NavPath path) {
        int minimum = context.searchConfig.pathCutoffMinimumLength();
        ...
        double factor = context.searchConfig.pathCutoffFactor();
        int lastPosition = (int) ((path.length() - minimum) * factor) + minimum - 1;
        return new CutoffPath(path, lastPosition);
    }
