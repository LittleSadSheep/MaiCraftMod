...
public final class FluidPlacementTaskRecord extends TaskRecord {
    ...
    public FluidPlacementTaskRecord(String callId, long deadline, BlockPos target, BlockState expected,
        ...
        this.sourceRegion = sourceRegion.stream().map(BlockPos::immutable).collect(java.util.stream.Collectors.toUnmodifiableSet());
        this.installation = installation.stream().map(BlockPos::immutable).collect(java.util.stream.Collectors.toUnmodifiableSet());
