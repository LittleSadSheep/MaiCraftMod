...
public class GoalBlock implements Goal {
    ...
    public double heuristic(int x, int y, int z) {
        ...
        int yDiff = y - this.y;
        int zDiff = z - this.z;
        return calculate(xDiff, yDiff, zDiff, costHeuristic);
    }
    ...
    public static double calculate(double xDiff, int yDiff, double zDiff) {
        ...
        heuristic += GoalXZ.calculate(xDiff, zDiff, NavSettings.get().costHeuristic);
        return heuristic;
    }

    public static double calculate(double xDiff, int yDiff, double zDiff, double costHeuristic) {
        double heuristic = GoalYLevel.calculate(0, yDiff);
        heuristic += GoalXZ.calculate(xDiff, zDiff, costHeuristic);
        return heuristic;
    }
