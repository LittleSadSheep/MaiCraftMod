...
public final class Avoidance {
     ...
     */
    public static List<Avoidance> forGoal(Goal goal, LocalPlayer player) {
     ...
     */
    public static List<Avoidance> create(LocalPlayer player) {
        ...
        if (mobCoeff != 1.0D) {
            ClientLevel level = requireClientLevel(player);
            // 只遍历客户端已经同步进视距的实体。
     ...
     */
    private static List<BlockPos> spawnersInLoadedChunks(LocalPlayer player, BlockPos center) {
    ...
    private static ClientLevel requireClientLevel(LocalPlayer player) {
        if (!(player.level() instanceof ClientLevel level)) {
            throw new IllegalArgumentException("avoidance scan requires ClientLevel");
        }
        return level;
    }
