...
final class SprintPolicy {
    ...
    /** Same authoritative jump-strength composition used by LivingEntity#getJumpPower. */
    private double jumpVerticalSpeed() {
    ...
    /** Live velocity plus vanilla's yaw-directed 0.2 sprint-jump impulse. */
    private Vec3 sprintJumpLaunchVelocity() {
    ...
    /**
     * Whether jumping on this exact grounded tick reaches the elevated cell without first
     * striking its vertical face, and descends onto either that cell or the one verified dry
     * landing cell behind it. Returning false means the early window is not open (or was missed);
     * the normal MovementAscend remains active and performs its ordinary close-range jump.
     */
    private boolean ascendLaunchReady(
            MovementTraverse current, MovementAscend next, Movement nextNext) {
        if (!(nextNext instanceof MovementTraverse landing)
                || !PathExecutor.playerFeet(player).equals(current.getSrc())) {
            return false;
        }
        Vec3i direction = current.getDirection();
        ...
        for (int tick = 0; tick < MAX_PROJECTED_AIRBORNE_TICKS; tick++) {
            ...
            if (!enteredHighPlatform && horizontal >= firstBodyOverlap) {
                // Combined movement may descend onto the top in this tick, but if neither end
                // of the vertical sweep reaches one block high, the body hits the side: the
                // momentum window has already been missed.
            ...
            if (enteredHighPlatform && descending && height >= 1.0 && nextHeight <= 1.0) {
                // Collision resolution meets the platform top from the pre-horizontal body
                // position of this combined movement tick; require that position to belong to
                // the verified two-cell landing strip.
