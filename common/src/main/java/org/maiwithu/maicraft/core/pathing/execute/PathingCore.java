...
public final class PathingCore {
    ...
    private void startSearch(BlockPos start) {
        ...
        Favoring favoring = new Favoring(current == null ? null : current.getPath(), context,
                org.maiwithu.maicraft.core.pathing.astar.Avoidance.forGoal(goal, player));
    ...
    private void noteSegmentEnd(PathExecutor segment, BlockPos feetBefore) {
        ...
        Constants.LOG.info(
                "[maicraft-path] 连续 {} 个路段被采纳即夭折且身位未动(段长 {},因由 {}),"
    ...
    private void countOutcome(String kind) {
        ...
        Constants.LOG.info("[maicraft-path-outcome] 派发{} 结论{} 目标={} 身位={} 段={}",
            ...
            if (executor.isPresent()) {
                if (executor.get().getPath().positions().contains(expectedSegmentStart)) {
                    ...
                    if (++orphanDiscards >= MAX_ORPHAN_DISCARDS) {
                        Constants.LOG.info(
                                "[maicraft-path] 连续 {} 次孤儿段:起点 {} 不在结果路径上"
                                        + "(路径起点 {},长 {}),判首段失败",
    ...
    private PathExecutor newExecutor(NavPath path) {
        ...
        var liveView = org.maiwithu.maicraft.core.pathing.cache.LoadedOnlyView.of(player.level());
        ChunkLoadedTest liveLoaded =
                liveView instanceof org.maiwithu.maicraft.core.pathing.cache.LoadedOnlyView v
