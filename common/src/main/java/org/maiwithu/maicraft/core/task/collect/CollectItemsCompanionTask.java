...
public final class CollectItemsCompanionTask extends AbstractCompanionTask<CollectItemsTaskRecord> {
    ...
    private TaskState tickScan() {
        ...
        target = best;
        targetItem = best.getItem().getItem();
        inventoryBefore = PlayerInv.count(player.getInventory(), targetItem);
    ...
    private TaskState tickApproach() {
        if (target == null || target.isRemoved()) {
            // Count only a synchronized inventory increase. Despawn or another player collecting
            // the entity is not our success.
            if (target != null && targetItem != null
                    && PlayerInv.count(player.getInventory(), targetItem) > inventoryBefore) {
