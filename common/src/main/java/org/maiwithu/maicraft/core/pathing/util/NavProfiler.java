...
public final class NavProfiler {
    ...
    // 客户端 tick 间隔脉冲:活跃任务每 tick 打一次点,相邻脉冲的间隔≈一个完整客户端 tick 的
    // 墙钟长度。它能抓到落在本探针各相位【之外】的主线程尖峰(光照、方块更新、未计时的任务
    // 工作等)——用于分辨寻路执行链与渲染/其他模组造成的卡顿。
    ...
    public static void clientTickPulse() {
    ...
    // ==================== 卡顿现场抓栈 ====================
    // 后台采样线程 25ms 一次抓客户端线程的调用栈存进环形缓冲;tick 间隔一超 100ms,
    // 就把卡顿区间内采到的栈打成 [nav-stall] 日志——直接指认客户端线程当时堵在哪个方法里,
    ...
    private static void flush(long now) {
        ...
        // 客户端 tick 间隔:频繁 >60/>100ms 说明游戏线程有探针相位外尖峰
        if (pulseGapMax > 0L) {
