...
public class MovementParkour extends Movement {
    ...
    public MovementParkour(LocalPlayer player, BlockPos src, BlockPos dest) {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (feet.equals(dest)) {
            ...
            if (feet.equals(src.relative(direction)) || player.getY() - src.getY() > 0.0001) {
                // 已跳出第一格或已离地
                if (mayAlterTerrain()
                        && MovementPlacement.selectForLocation(
                                player, dest.below(), false, itemSelector())
                                != ItemSelection.UNAVAILABLE
                        && !MovementHelper.canWalkOn(player.level(), dest.below())
                        && !player.onGround()
                        && MovementPlacement.attemptToPlaceABlock(
                                state, player, dest.below(), true, false, itemSelector())
