...
public final class BuildingSceneResources {
    ...
    static KnowledgeDocument read(String uri) {
        ...
        if (parts.length != 2 || !Set.of("scene", "blueprint").contains(parts[0])) throw KnowledgeException.missing(uri);
        ...
        var document = parts[0].equals("scene") ? entry.scene()
                : BuildingSceneBlocks.export(BuildingSceneCompiler.compile(entry.scene()));
