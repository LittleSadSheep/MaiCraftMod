...
/** Draw to the main target, with depth testing but without writing ghost depth. */
final class PreviewRenderTypes extends RenderType {
    static final RenderType GHOST = new PreviewRenderTypes("maicraft_preview_model", DefaultVertexFormat.BLOCK,
            VertexFormat.Mode.QUADS, true, List.of(RENDERTYPE_TRANSLUCENT_SHADER, BLOCK_SHEET_MIPPED,
                    TRANSLUCENT_TRANSPARENCY, NO_CULL, LIGHTMAP, COLOR_WRITE, LEQUAL_DEPTH_TEST, MAIN_TARGET));
    ...
    static final RenderType PARTS = new PreviewRenderTypes("maicraft_preview_parts", DefaultVertexFormat.POSITION_COLOR,
            VertexFormat.Mode.QUADS, true, List.of(POSITION_COLOR_SHADER, TRANSLUCENT_TRANSPARENCY,
                    NO_CULL, COLOR_WRITE, LEQUAL_DEPTH_TEST, MAIN_TARGET));
