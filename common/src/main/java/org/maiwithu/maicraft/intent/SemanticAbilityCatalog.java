    ...
    private static JsonObject describeContract(String ability) {
        return switch (ability) {
            ...
            case MachineAbilityAdapter.DESIGN -> contract(
                    "Optional machine review without construction. For a build request, normally read the actual process, perceive construction_site, author an explicit blueprint with assembly, then plan/execute build_machine directly. Supply exactly one of blueprint, blueprint_uri or legacy design. The author chooses components, work surfaces and transport; consult machine_assembly and selected block documents for concrete missing details. Materials, live power and measured output are not prerequisites to draft or review. Preserve expected_output and forbidden_mods; a review never claims completed construction or production.",
            ...
            case MachineAbilityAdapter.BUILD -> contract(
                    "Construct the authored blueprint at construction_site's target and snapshot_id. Normal workflow: read process resources, perceive construction_site, author blueprint, plan then execute its plan_id. plan validates native installation; no separate design_machine or material investigation is required. MaiCraft supplies construction materials, checks the full footprint and verifies native placement. With production and allow_use=true, run the declared v1 network or v2 native process afterward. Dev mode waits for local preview confirmation.",
                    targets("landmark", "area"),
                    fields(
                            field("snapshot_id", "string", "Use construction_site's receipt, or a fresh complete inspect_machine receipt. Site receipts survive design time while observed geometry stays unchanged; execution rechecks the geometry and consumes the receipt. The Mod inspects the complete construction footprint before placing blocks."),
