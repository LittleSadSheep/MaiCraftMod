    ...
    private static JsonObject describeContract(String ability) {
        return switch (ability) {
            ...
            case MachineAbilityAdapter.BUILD -> contract(
                    ...
                    fields(
                            field("snapshot_id", "string", "Copy construction_site's snapshot_id. Reuse the same-session anchor through construction and retries; the executor reads current target blocks. Refresh after a session change. A fresh complete inspect_machine receipt is also accepted but retains its short lifetime."),
