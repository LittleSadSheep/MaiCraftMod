    ...
    private static JsonObject describeContract(String ability) {
        return switch (ability) {
            ...
            case MachineAbilityAdapter.BUILD -> contract(
                    ...
                    fields(
                            field("snapshot_id", "string", "Copy construction_site's snapshot_id. In the same session, unchanged site geometry permits reuse through design and retries before any placement; each attempt rechecks geometry. Refresh only after a site diagnostic or session change. A fresh complete inspect_machine receipt is also accepted but retains its short lifetime. MaiCraft checks the full footprint before placing blocks."),
