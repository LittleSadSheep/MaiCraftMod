        ...
        return switch (ability) {
            ...
            case "maicraft:build", BuildDesignAdapter.ABILITY -> contract(
                    BuildDesignAdapter.ABILITY.equals(ability)
                            ...
                            : "Construct a model authored by the LLM, or request a semantic structure. Model operations stay inside this build ability: create_scene, update_scene, get_scene_info, get_object_info, get_component_info, preview, export_scene and build. Only build changes the Minecraft world. Keep named objects, transforms, material slots and Boolean modifiers like Blender; the Mod voxelizes and executes the final cells. This is a declarative modelling subset, not a Python/bpy interpreter. Reuse scene_id for edits/preview/build and project_id for interrupted construction; never replan current_place to resume.",
                    targets("area", "landmark", "coordinates", "current_place", "prior_result"),
                    fields(
                            // 编辑产生新模型后，可明确把同一坐标范围的修订交给旧项目；采用修订与真正施工分开。
                            field("operation", "string", "Model operation: create_scene, update_scene, get_scene_info, get_object_info, get_component_info, preview, export_scene, revise_project or build (default). revise_project adopts a direct child scene revision for an idle project, preserving its policies and verified scaffolds; it changes no world blocks. These run through plan/execute; modelling operations never start body work. No operation keeps legacy semantic construction."),
                            // 模型先完整展开和校验，再进入既有预览/供料/施工；组件定义本身不会放进世界。
                            field("scene", "object", "LLM-authored scene; v1 cube/panel semantics remain supported. v2: {schema_version:2,coordinate_system:'minecraft_y_up',materials:{Wall:{block_id:'minecraft:oak_planks'}},components:{Column:{objects:[{name:'Shaft',type:'MESH',primitive:'cube',location:[0.5,1.5,0.5],dimensions:[1,3,1],material:'Wall'}]}},objects:[{name:'Columns',type:'INSTANCE',component:'Column',location:[0,0,0],array:{count:[4,1,1],step:[3,0,0],skip:[[1,0,0]]}}]}. MESH and INSTANCE allow array, quarter-turn rotation_euler on all axes and mirror:['x']; INSTANCE also supports material_map. MESH supports triangle, wedge, triangular_prism, tetrahedron/triangular_pyramid, pyramid, prism, cylinder, cone, convex_polyhedron, fill:solid/hollow, wall_thickness, open_faces, face_materials, edge_material/edge_materials and edge_width. v2 block_state_axes defaults to local; minecraft_world preserves world-facing states. overlap_policy:last_wins/error. Blender coordinates remain supported. Root objects and their referenced definitions generate geometry; unused component definitions do not build. Cycles, invalid nested fields, material/face names and expansion budgets reject before construction. See blueprint knowledge for complete conventions."),
