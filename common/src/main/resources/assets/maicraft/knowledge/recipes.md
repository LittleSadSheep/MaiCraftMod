...
本页处理“获得材料”的工艺规划。玩家明确要求新建机器时，直接按场地感知、生成蓝图、`plan`、`execute` 的建造流程推进，施工取料交给 Mod；按需查配方不会额外产生遍查库存和已有设备的前置要求。
...
3. 缺设备时按工作站物品读取方块状态、说明与 Ponder，勘测场地后生成显式 `blueprint`，通过 `plan` 校验并 `execute` 交给 Mod 施工；`design_machine` 是可选审阅。Ponder 场景中的创造资源源头表示资源 IN，应接入真实网络；虚构转速、直接生成物品和动画不证明复制出的结构能工作。
