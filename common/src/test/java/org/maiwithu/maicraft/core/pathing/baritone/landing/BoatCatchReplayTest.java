...
public final class BoatCatchReplayTest {
    public static void main(String[] args) throws Exception {
        ...
        f.player.inventory.setItem(0,new ItemStack(Items.OAK_BOAT));
        var spawn=new Vec3(.5,0,.5);
        var controller=new BoatLandingAssist(new BoatLandingSnapshot.Plan(new BlockPos(0,12,0),BlockPos.ZERO,
                Items.OAK_BOAT,null,spawn,true),false);
        ...
        var actions=(NativeActionPort)Proxy.newProxyInstance(NativeActionPort.class.getClassLoader(),new Class<?>[]{NativeActionPort.class},
                (proxy,method,values)->switch(method.getName()) {
                    case "useItem" -> {
                        ...
                        entity[0]=new Boat(f.world,spawn.x,spawn.y,spawn.z);
                        field(Entity.class,"onGround").setBoolean(entity[0],true);
                        entity[0].setDeltaMovement(Vec3.ZERO); f.world.observedEntities=List.of(entity[0]);
                        f.player.inventory.setItem(0,ItemStack.EMPTY); placed[0]++;
                    ...
                    case "interact" -> {
                        ...
                        field(Entity.class,"vehicle").set(f.player,entity[0]);
                        field(Entity.class,"passengers").set(entity[0],com.google.common.collect.ImmutableList.of(f.player));
                        f.player.fallDistance=0; mounted[0]++;
        ...
        f.position(2.7,-.9,false); f.player.setYRot(52); f.player.setXRot(0); f.player.fallDistance=10;
        controller.tick(context);
        check(placed[0]==1 && !controller.failed(),"urgent aim submits one boat even in a short native opportunity");
        f.position(1.8,-.96,false); f.time++; controller.tick(context);
