...
public final class TargetedDropEvidenceTest {
    ...
    private static void geometryIsReadOnly() throws Exception {
        try (var world = new InteractionWorldTestHarness()) {
            ...
            var cells = new java.util.ArrayList<BlockPos>();
    ...
    private static void exactRegionAndNativeBounds() throws Exception {
        try (var world = new InteractionWorldTestHarness()) {
            ...
            for (BlockPos cell : java.util.List.of(receiver,included,hole)) world.set(cell,Blocks.WATER.defaultBlockState());
            ...
            var region = TargetedDropRegion.ofCells(java.util.List.of(receiver,included))
                    .narrowTo(new net.minecraft.world.phys.AABB(6.1,1,6.1,6.6,2,6.6));
            var type=Class.forName("org.maiwithu.maicraft.core.task.inventory.TargetedDropReceipt");
            var constructor=type.getDeclaredConstructor(net.minecraft.client.player.LocalPlayer.class,BlockPos.class,ItemStack.class,int.class,TargetedDropRegion.class);
            ...
            ActorControlTestHarness.field(net.minecraft.world.entity.Entity.class,"position").set(item,new Vec3(5.5,1,6.5));
            ...
            ActorControlTestHarness.field(net.minecraft.world.entity.Entity.class,"position").set(item,new Vec3(6.8,1,6.8));
            check(receipt.observe(context)==NativeConfirmation.Verdict.PENDING,"在声明格内却越过取物约束的实体不能提前确认");
            ActorControlTestHarness.field(net.minecraft.world.entity.Entity.class,"position").set(item,new Vec3(6.4,1,6.4));
    ...
    private static void finalGuardDoesNotDrop() throws Exception {
        try (var world = new InteractionWorldTestHarness()) {
            var menu = new net.minecraft.world.inventory.InventoryMenu(world.inventory,false,world.player);
            ActorControlTestHarness.field(net.minecraft.world.entity.player.Player.class,"inventoryMenu").set(world.player,menu);world.player.containerMenu=menu;
            var cells=new java.util.ArrayList<BlockPos>();
            ...
            check(world.h.connection.packets.stream().noneMatch(packet->packet instanceof net.minecraft.network.protocol.game.ServerboundPlayerActionPacket action
                    ...
                    ||action.getAction()==net.minecraft.network.protocol.game.ServerboundPlayerActionPacket.Action.DROP_ALL_ITEMS)),"守卫失败不能发出原生Q");
